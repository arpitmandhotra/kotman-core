...
func main() {
	...
	if err != nil {
		...
		// Look up the config key directly from the map
...
func (w *RecoveryWorker) processCartRecovery(ctx context.Context, merchantID, phoneHash string) {
    ...
    // 2. Retrieve raw phone from data key (set at cart abandonment time)
    ...
    // 3. Load trust profile (may not exist for new buyers)
    profile, profileErr := w.loadProfile(phoneHash)

    // 4. Risk gate — skip high-risk buyers entirely
    if profileErr == nil {
    ...
    // 5. Choose template and build event
    ...
    // 6. Route to CRM or fallback to direct WhatsApp
...
func (w *RecoveryWorker) executeRouting(
    ...
    if settings.HasOwnCommunicationsKey && settings.ProviderAPIKey != "" {
        ...
        w.sendWhatsApp(rawPhone, phoneHash, event.Template, event.DiscountValue, settings.ProviderAPIKey)
        return
    }
    ...
    if settings.WalletBalance >= messageCost {
        ...
        // Atomic wallet deduction — only deducts if balance is sufficient
        ...
        w.sendWhatsApp(rawPhone, phoneHash, event.Template, event.DiscountValue, masterKey)
        return
    }
...
func (w *RecoveryWorker) fetchAndDeleteDataKey(ctx context.Context, dataKey string) (string, error) {
    ...
    if err == redis.Nil {
        slog.Warn("data key missing — trigger fired but data already expired", "key", dataKey)
        return "", err
    }
    if err != nil {
    ...
    // Delete immediately — PII should not linger
...
func (w *RecoveryWorker) computeRiskScore(profile *domain.TrustProfile) float64 {
    ...
    score := 85.0
    score += profile.RiskAdjustment // negative deltas from feedback weights
    if profile.TotalOrders > 0 {
        rtoRate := float64(profile.TotalRTOs) / float64(profile.TotalOrders)
        score -= rtoRate * 40 // high RTO rate pulls score down
    }
    if score < 0 {
        score = 0
    }
...
func (w *RecoveryWorker) sendWhatsApp(rawPhone, phoneHash, template string, discount int, apiKey string) {
    // Mask phone for logs — never log raw PII
    masked := "****"
    if len(rawPhone) > 6 {
        masked = rawPhone[:4] + "****" + rawPhone[len(rawPhone)-2:]
    }

    // Verify the key is valid using canonical hash (not phone normaliser)
    // Only log the provider hint, never key material
    _ = crypto.HashPhone(rawPhone) // ensure crypto package is used, validates import
    ...
    // Production: POST to Twilio/Interakt API here using apiKey
}