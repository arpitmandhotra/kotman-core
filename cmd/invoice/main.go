...
const invoiceWorkerCount = 5 // process 5 merchants concurrently

type invoiceJob struct {
	acc domain.MerchantBillingAccumulator
}
...
func main() {
	...
	var invoicesCreated int64 = 0
	var totalAmountBilled int64 = 0
	...
	jobs := make(chan invoiceJob, len(accumulators))
	var wg sync.WaitGroup

	for i := 0; i < invoiceWorkerCount; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			for job := range jobs {
				processInvoice(pg, job.acc, billingMonth, billingPeriodStart, billingPeriodEnd, &invoicesCreated, &totalAmountBilled)
			}
		}()
	}

	for _, acc := range accumulators {
		jobs <- invoiceJob{acc: acc}
	}
	close(jobs)
	wg.Wait()

	slog.Info("completed monthly invoice generation job",
		"invoices_created", atomic.LoadInt64(&invoicesCreated),
		"total_amount_billed_paise", atomic.LoadInt64(&totalAmountBilled),
		"month", billingMonth,
	)
}
...
func processInvoice(
	...
	err := pg.Transaction(func(tx *gorm.DB) error {
		...
		if domain.IsGrowthOrAbove(merchant.Tier) {
			...
			// Business decision: WhatsApp surcharge waived for RTO Engine subscribers — see pricing spec v2
		...
		atomic.AddInt64(invoicesCreated, 1)
		atomic.AddInt64(totalAmountBilled, int64(invoice.TotalFeePaise))
		return nil
	})
